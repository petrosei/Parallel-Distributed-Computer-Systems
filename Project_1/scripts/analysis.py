...
path = "../Results/cilkplus_parallel_results/"
...
for q in Q:
        ...
        rec = []
        for n_proc in processors:
                filename = "{0}/cilkplusresults_{1}_{2}.txt".format(path,n_proc,q)
                file = open(filename, "r")
                res =  file.read().splitlines()
                res1 = res[0].split()
                res2 = res[1].split()
...
path = "../Results/pthreads_parallel_results/"
...
for q in Q:
        ...
        rec = []
        for n_proc in processors:
                filename = "{0}/pthreadsresults_{1}_{2}.txt".format(path,n_proc,q)
                file = open(filename, "r")
                res =  file.read().splitlines()
                res1 = res[0].split()
                res2 = res[1].split()
